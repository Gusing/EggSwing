...
public class enemyParryPracticeTutorialHandler : enemyHandler {
    ...
    public override void Update()
    {
        ...
        if (dead)
        {
            Destroy(gameObject, GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).length + 0.3f);
        }
        ...
        // check for attack
        if (Vector2.Distance(transform.position, player.transform.position) < 3f && !attacking && !fallFromAbove && !hitstun && attackRecovery <= 0 && !parried && !falling)
        {
