...
public class playerHandlerTutorial : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!busy && mainHandlerTutorial.songStarted && mainHandlerTutorial.currentGameMode != 1 && !normalLevelFinished)
        {
            if (Input.GetButtonDown("Heavy Attack") && !disabledHeavy)
            {
                ...
                attackIDStart = attackID;
                Punch();
                if (((beatState == SUCCESS || beatState == BEAT) && !restrictedHeavy) ||
            (restrictedHeavy && beatIndicator.GetComponent<beatIndicatorHandlerBTutorial>().otherBeat && beatState == BEAT) ||
            (restrictedHeavy && !beatIndicator.GetComponent<beatIndicatorHandlerBTutorial>().otherBeat && beatState == SUCCESS))
                    beatIndicator.GetComponent<beatIndicatorHandlerBTutorial>().PlayerInput();
                else beatIndicator.GetComponent<beatIndicatorHandlerBTutorial>().PlayerInput(false, true);
            }
    ...
    void Punch()
    {
            ...
            (restrictedHeavy && !beatIndicator.GetComponent<beatIndicatorHandlerBTutorial>().otherBeat && beatState == SUCCESS))
        {
            ...
            */
            ...
            gameManager.PlayerInput(1, true);

            SuccessfulPunch();
        }
        else if (beatState == FAIL || restrictedHeavy)
        {
    ...
    void Hitstun()
    {
        ...
        if (lastDmgRecieved > 0) velX = (-3f - actionTimer * 5) * hitstunDirection;
